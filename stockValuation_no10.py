...
fs = pd.read_excel('raw/Stock_2017.xlsx', encoding='utf-8', skiprows=2, sheetname="Financial Ratio").T
...
def valuation_stock(s_name):
    ...
    percent = [0.03, 0.05, 0.07]
    target_price_g = [dividendYield * (1 + p) / (K - p) for p in percent]
    #print('Price by g=3%: {}\nPrice by g=5%: {}\nPrice by g=7%: {}'
    #            .format(target_price_g[0].iloc[0], target_price_g[1].iloc[0], target_price_g[2].iloc[0]))
    ...
    #print('P/E by g=3%: {}\nP/E by g=5%: {}\nP/E by g=7%: {}'
    #            .format(target_pe_g[0].iloc[0], target_pe_g[1].iloc[0], target_pe_g[2].iloc[0]))
    target_price_pe_g = [EPS * (1 + p) * t for t, p in zip(target_pe_g, percent)]
    #print('Price by P/E g=3%: {}\nPrice by P/E g=5%: {}\nPrice by P/E g=7%: {}'
    #            .format(target_price_pe_g[0].iloc[0], target_price_pe_g[1].iloc[0], target_price_pe_g[2].iloc[0]))
    ...
    #print('P/BV by g=3%: {}\nP/BV by g=5%: {}\nP/BV by g=7%: {}'
    #            .format(target_bv_g[0].iloc[0], target_bv_g[1].iloc[0], target_bv_g[2].iloc[0]))
    target_price_bv_g = [t * BV * (1 + p) for t, p in zip(target_bv_g, percent)]
    #print('Price by P/BV g=3%: {}\nPrice by P/BV g=5%: {}\nPrice by P/BV g=7%: {}'
    #            .format(target_price_bv_g[0].iloc[0], target_price_bv_g[1].iloc[0], target_price_bv_g[2].iloc[0]))

    data = {'Detail': ['Price by Dividend Discount Model', 'Price by g=3%', 'Price by g=5%', 'Price by g=7%',
                       'Price by PE = %.2f' % target_pe.iloc[0], 'Price by MaxPE', 'Price by AvgPE', 'Price by MinPE',
                       'Price by P/E g=3%', 'Price by P/E g=5%', 'Price by P/E g=7%',
                       'Price by BV = %.2f' % target_bv.iloc[0], 'Price by MaxBV', 'Price by AvgBV', 'Price by MinBV',
                       'Price by P/BV g=3%', 'Price by P/BV g=5%', 'Price by P/BV g=7%'
            ],
            'Price': [target_price_dy.iloc[0], target_price_g[0].iloc[0], target_price_g[1].iloc[0], target_price_g[2].iloc[0],
                      target_price_pe.iloc[0], maxPE_price, avgPE_price, minPE_price,
                      target_price_pe_g[0].iloc[0], target_price_pe_g[1].iloc[0], target_price_pe_g[2].iloc[0],
                      target_price_bv.iloc[0], maxBV_price, avgBV_price, minBV_price,
                      target_price_bv_g[0].iloc[0], target_price_bv_g[1].iloc[0], target_price_bv_g[2].iloc[0]
            ]
    }
    ...
    print("Price average = {:,.2f}".format(price_avg))
    print("Price at average PE {:,.2f} = {:,.2f}".format(avgPE, avgPE_price))
    print("Price at average BV {:,.2f} = {:,.2f}".format(avgBV, avgBV_price))
    #plot_price(price_table)

    return price_avg, price_table

def plot_price(price_table):
    ...
    plt.xlabel("Index")
    plt.ylabel("Price in particular P/E")
    plt.show()

if __name__ == "__main__":
    price_avg, price_table = valuation_stock('AHC')
    #print(price_avg)
    plot_price(price_table)